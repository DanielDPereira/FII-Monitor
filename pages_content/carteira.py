...
def _tab_historico():
    ...
    ultima_edicao_id = st.session_state.get("ultima_transacao_editada_id")
    ultima_edicao_em = st.session_state.get("ultima_transacao_editada_em")
    if ultima_edicao_id and ultima_edicao_em:
        st.info(
            f"Última transação editada: ID {ultima_edicao_id} às {ultima_edicao_em}.",
            icon="✅",
        )
    ...
    with st.expander("✏️ Editar Transação", expanded=False):
